    ...
    >
      ...
      <div className="flex items-center gap-2">
        ...
        <span className="text-[11px] font-data" style={{ color: "var(--text-muted)" }}>
          {t("progress", { done: plan.tsd, total: totalSwaps, left: balBTC.toFixed(4) })}
        </span>
      </div>
  ...
  const confirmCancel = () => {
    ...
    withLoading(() =>
      cancelSBTCPlan(plan.id,
        ({ txId }) => {
          setLoading(false);
          addNotification(t("cancelledToast"), "success", "dca-out", 5000,
            { planId: String(plan.id), action: "cancelled", amount: balBTC.toFixed(8), txId });
          onRefresh();
        },
        () => {
          setLoading(false);
          addNotification(t("cancelFailed"), "error", "dca-out", 5000);
        },
      ),
    );
  };
  ...
  const handleExecute = () => {
    ...
    withLoading(() =>
      executeSBTCPlan(plan.id, routerInput.trim(), minAmountOut,
        ({ txId }) => {
          setLoading(false);
          addNotification(t("executedToast"), "success", "dca-out", 5000,
            { planId: String(plan.id), txId, action: "executed" });
          onRefresh();
        },
        () => {
          setLoading(false);
          addNotification(t("executionFailed"), "error", "dca-out", 5000, { planId: String(plan.id) });
        },
      ),
    );
  };
            ...
            <div className="border-t p-4 flex flex-col gap-3" style={{ borderColor: "var(--border-subtle)" }}>
              ...
              <div className="grid grid-cols-3 gap-2 text-center">
                {[
                  ...
                  { label: t("sbtcSpent"), value: satsToBTC(plan.tss).toFixed(8) },
                  ...
                  <div className="rounded-lg p-2.5 flex flex-col gap-2" style={{ background: "var(--bg-card)", border: "1px solid var(--border-subtle)" }}>
                    <div className="flex items-center justify-between">
                      <span className="text-[10px] font-medium" style={{ color: "var(--text-secondary)" }}>
                        {t("netSwap", { amount: satsToBTC(netSats).toFixed(8) })}
                      </span>
