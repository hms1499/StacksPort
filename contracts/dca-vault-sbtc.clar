;; dca-vault-sbtc: Dollar-Cost Averaging vault using sBTC as source token
;; Swaps sBTC into target tokens (e.g. USDCx) via pluggable swap routers

;; DCA Swap Router trait - router receives sBTC, swaps to target token, sends to recipient
...
(define-public (execute-dca
  ...
  (let ((p    (unwrap! (map-get? plans plan-id) E101))
    ...
    ;; Transfer 0.3% protocol fee in sBTC to treasury
    (as-contract (try! (contract-call?
      'SM3VDXK3WZZSA84XXFKAFAF15NNZX32CTSG82JFQ4.sbtc-token
      transfer pf tx-sender TREASURY none)))
    ;; Transfer remaining 99.7% sBTC to swap router
    (as-contract (try! (contract-call?
      'SM3VDXK3WZZSA84XXFKAFAF15NNZX32CTSG82JFQ4.sbtc-token
      transfer net tx-sender (contract-of swap-router) none)))
    ;; Router swaps sBTC and sends target tokens to plan owner
