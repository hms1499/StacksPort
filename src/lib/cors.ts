 ...
 *
 * Kept separate from `middleware.ts` so it can be unit-tested without pulling
 * in `next-intl/middleware` (which imports `next/server` internals that the
 * test runtime cannot resolve).
 */
...
export function handleApiCors(req: NextRequest): NextResponse {
