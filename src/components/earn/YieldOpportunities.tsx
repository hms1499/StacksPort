...
function YieldOpportunities() {
  ...
  // Split into "active" (already running — manage) and "available" (discover).
  // Keeps every action reachable while removing the contradiction of a
  // position being shown as both active and a discovery opportunity.
  const active = opportunities.filter((o) => o.status === "active");
  const available = opportunities.filter((o) => o.status === "available");

  function renderRow(o: Opportunity) {
    const Icon = o.icon;
  ...
  return (
    <div className="glass-card rounded-2xl p-5 shadow-sm">
      ...
      {active.length > 0 && (
        <div className="mb-4">
          {sectionLabel(tz("opportunities.activeTitle"))}
      ...
      {available.length > 0 && (
        <div>
          {/* Only label the Explore group when an Active group sits above it. */}
          {active.length > 0 && sectionLabel(tz("opportunities.exploreTitle"))}
