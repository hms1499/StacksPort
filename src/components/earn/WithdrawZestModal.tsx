// src/components/earn/WithdrawZestModal.tsx
"use client";
...
export default function WithdrawZestModal({ open, onClose, suppliedSbtc }: Props) {
  ...
  const errorText =
    amountSats === 0
      ...
      : validation.ok
      ? null
      : validation.reason === "exceeds-supplied"
      ? t("zest.errExceeds")
      : t("zest.errZero");
  ...
  const handleSubmit = async () => {
    ...
    setLoading(true);
    // The withdraw call needs the user's full collateral-reserve set for the
    // health-factor calc; resolve it before opening the wallet.
    const reserves = await readUserCollateralReserves(stxAddress);
    withdrawZestSbtc(
      amountSats,
      stxAddress,
      reserves,
  ...
  const setMax = () => setAmount(String(suppliedSbtc));
      ...
      <div className="glass-card rounded-2xl p-5 w-full max-w-sm flex flex-col gap-4" style={{ boxShadow: "var(--shadow-card)" }} onClick={(e) => e.stopPropagation()}>
        <div className="flex items-start justify-between">
          <div className="flex items-center gap-2">
            ...
            <div>
              <p className="font-semibold" style={{ color: "var(--text-primary)" }}>{t("zest.withdrawTitle")}</p>
            </div>
          </div>
          ...
          <>
            <div className="flex flex-col gap-1">
              ...
              <p className="text-[11px]" style={{ color: "var(--text-muted)" }}>
                {t("zest.supplied")}: {suppliedSbtc.toFixed(8)} sBTC
              </p>
            </div>
            ...
            >
              {loading ? t("zest.pending") : t("zest.withdrawCta")}
            </button>
          </>
        )}
      </div>
