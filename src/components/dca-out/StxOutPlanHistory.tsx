...
// USDCx is the only STX-out target token; hardcode 6 decimals.
...
function statusColor(status: StxUsdcxExecutionEvent["status"]): string {
...
export default function StxOutPlanHistory({ plan }: Props) {
  const t = useTranslations("dca.out.history");
  const [items, setItems] = useState<StxUsdcxExecutionEvent[] | null>(null);
  ...
  const fetchHistory = useCallback(async () => {
    ...
    setError(null);
    try {
      const data = await getStxUsdcxPlanExecutionHistory(plan.id, plan.token, 100, plan.owner);
      setItems(data);
    } catch (e) {
        ...
        >
          ...
          {ev.status === "success" && (
            <div className="flex flex-col items-end gap-0.5">
              {ev.stxIn !== undefined && (
                <span className="text-xs font-semibold font-data" style={{ color: "var(--text-primary)" }}>
                  {microToSTX(ev.stxIn).toFixed(2)} STX
                </span>
              )}
